...
def animate_dataset():
    ...
    # ,label="Constraints")
    line1, = ax_x_0.plot([], [], color='orange', lw=0.8)
    # ,label="Tightened pseudo constraints")
    line2, = ax_x_0.plot([], [], color='green', lw=0.8, ls='--')
    line3, = ax_x_0.plot([], [], lw=0.8, label="Reference trajectory")
    line4 = ax_x_0.scatter([], [], color='grey', s=0.1,
                           label="Nominal prediction horizon")
    line5 = ax_x_0.scatter([], [], color='black', s=0.2,
                           label="Prediction horizon")
    line6 = ax_x_0.scatter([], [], color='red', s=0.1,
                           label="Measured positions $x$")

    # ax_x_0.legend(loc="right",prop=without_serif)

    line_est_pdf, = ax_distr.plot(
        [], [], lw=0.8, ls='--', label=r"$f_\mathrm{est}(\Delta x)$")
    # line_true_pdf, = ax_distr.plot([],[],lw=0.8,label=r"$f_\mathrm{true}(\Delta x)$")
    ax_distr.legend(prop=without_serif)

    line_weights, = ax_weights.plot(
        [], [], color='black', lw=0.8, label=r"Weights $w_i$")
    ax_weights.legend(prop=without_serif)

    bar_b_coeff, = ax_b_coeff.bar(1, 1, label=r"$b_c$")
    ax_b_coeff.xaxis.set_ticklabels([])
    bar_b_coeff_text = ax_b_coeff.text(
        1, .5, '', va="center", ha="center", fontsize=8, color="black")
    ...
    true_pdf = 1/(true_std_dev*np.sqrt(2*np.pi))*np.exp(-0.5 *
                                                        np.power((pdf_interval-true_mean)/true_std_dev, 2))

    def animate(k):

        path_measured_state = os.path.join(
            path_dataset, "mpc", "measured_states", "real_state_k_"+str(k)+".npy")
        ...
        path_g_z = os.path.join(path_dataset, "constraint_tightening",
                                "tightened_constraints", "g_z_k_"+str(k)+".npy")
        ...
        # Plot distribution
        path_estim_pdf = os.path.join(path_dataset, "disturbance_estimation",
                                      "disturbance_distribution_x_0", "estim_pdf_k_"+str(k)+".npy")
        estimated_pdf = np.load(path_estim_pdf)
        resize_estimated_pdf = estimated_pdf * \
            number_eval_points/(interv_max-interv_min)
        ...
        path_weights = os.path.join(
            path_dataset, "disturbance_estimation", "weights", "weights_k_"+str(k)+".npy")
        ...
        path_est_trajectory = os.path.join(
            path_dataset, "mpc", "optimal_trajectories", "trajectory_k_"+str(k)+".npy")
        ...
        path_b_coeff = os.path.join(path_dataset, "disturbance_estimation",
                                    "bhattacharyya_coefficients", "b_coeff_k_"+str(k)+".npy")
        ...
        if k == 0 or k == 50 or k == 100 or k == 150 or k == 200 or k == 250 or k == 300 or k == 399 or k == 400 or k == 449 or k == 499:
            path_cur_plot = os.path.join(
                path_plots, name_dataset+"_k_"+str(k)+".pdf")
