...
name_dataset = "gaussian_gaussian_only_x1"
len_traj = 500
...
def animate_dataset():
    ...
    true_pdf_first_250 = 1/(0.15*np.sqrt(2*np.pi))*np.exp(-0.5*np.power((pdf_interval+0.15)/0.15,2))
    true_pdf_last_250 = 1/(0.1*np.sqrt(2*np.pi))*np.exp(-0.5*np.power((pdf_interval-0.1)/0.1,2))
