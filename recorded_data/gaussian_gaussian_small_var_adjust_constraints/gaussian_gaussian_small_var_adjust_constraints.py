...
name_dataset = "gaussian_gaussian_small_var_adjust_constraints"
len_traj = 500
...
def animate_dataset():
    ...
    line4 = ax_x_0.scatter([], [], color='black', lw=2)
    ...
    true_pdf_first_250 = 1/(0.02*np.sqrt(2*np.pi))*np.exp(-0.5*np.power((pdf_interval+0.15)/0.02,2))
    true_pdf_last_250 = 1/(0.04*np.sqrt(2*np.pi))*np.exp(-0.5*np.power((pdf_interval-0.1)/0.04,2))
    ...
    def animate(k):
        ...
        if(k<250):
            true_pdf = true_pdf_first_250
        else:
            true_pdf = true_pdf_last_250
