...
name_dataset = "final_presentation_showcase"
len_traj = 450
...
def animate_dataset():
    ...
    line1, = ax_x_0.plot([], [], color='orange', lw=5,label="Constraints")
    line2, = ax_x_0.plot([], [], color='green', lw=5, ls='--',label="Tightened pseudo constraints")
    line3, = ax_x_0.plot([], [], lw=5,label="Reference trajectory")
    line4 = ax_x_0.scatter([], [], color='grey', s=30,label="Nominal prediction horizon")
    line5 = ax_x_0.scatter([], [], color='black', s=40,label="Prediction horizon")
    line6 = ax_x_0.scatter([], [], color='red', s=30,label="Measured positions $x$")
    ...
    true_pdf = 1/(0.02*np.sqrt(2*np.pi))*np.exp(-0.5*np.power((pdf_interval-0.15)/0.02,2))
    ...
    def animate(k):
        ...
        ax_x_0.scatter(k, measured_state[0], s=30,color='red')
        ...
        ax_distr.set_ylim(0,max(np.max(true_pdf),np.max(resize_estimated_pdf))+1)
