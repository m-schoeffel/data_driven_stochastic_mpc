...
class DiscountedKDE:
    ...
    def get_disturbance_intervals(self):
        ...
        for i in range(0, self.number_of_states):
            ...
            prob_distr_integr = np.cumsum(prob_distr) * (x_high-x_low)/number_eval_points

            # Get bounds for interval, in which P(disturbance)>p, e.g. [(p/2),1-(p/2)]
            idx_lower_bound = np.searchsorted(prob_distr_integr,self.p/2,side='left')
            idx_upper_bound = np.searchsorted(prob_distr_integr,1-(self.p/2),side='right')-1
